...
<AlarmEkrani>:
    ...
    MDFloatLayout:
        ...
        MDCard:
            ...
            CustomInput:
                id: a_period
                label: "Kaç adet ve kaç gün alacaksınız?"
                text_val: "Her Gün"
...
class BaseMenuScreen(MDScreen):
    ...
    yillar = [str(i) for i in range(current_year, 2020, -1)]
    temp_day = "01"
    temp_month = "01"
...
class AlarmEkrani(BaseMenuScreen):
    ...
    def on_time_save(self, instance, time):
        ...
        self.ids.a_time.ids.text_field.text = saat_str

    def open_period_menu(self):
        secenekler = ["Her Gün", "İki Günde Bir", "Haftada Bir"]
        self.open_dropdown(self.ids.a_period.ids.text_field, secenekler, self.set_val)

    def kaydet(self):
        secilen_ilac = self.ids.a_name.ids.text_field.text
        yeni_saat = self.ids.a_time.ids.text_field.text
        
        if secilen_ilac == "Seçiniz" or secilen_ilac == "Önce İlaç Ekleyin":
            print("Lütfen ilaç seçin")
            return
            
        print(f"Alarm Kuruldu: {secilen_ilac} - {yeni_saat}")
        self.manager.current = "dashboard"
